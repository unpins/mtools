...
- **macOS / Windows:** mtools operates on FAT *image files* (and, on Linux, block devices). `charsetConv.c`'s `iconv` use is linked statically on macOS. The Windows build is a [Cosmopolitan](https://github.com/jart/cosmopolitan) `.exe` (`cosmocc` → APE), with cosmo's lowercase `privileged` attribute macro neutralized where it collides with mtools' `SimpleFile_t.privileged` field — see [`cosmo.nix`](cosmo.nix).
